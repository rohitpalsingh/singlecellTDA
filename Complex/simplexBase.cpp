...
std::vector<std::set<simplexNode_P, cmpByWeight>> simplexBase::getDimEdges(int dim){
	if(dim >= simplexList.size()){
		ut.writeLog(simplexType,"Error: requested dimension beyond complex");
		std::vector<std::set<simplexNode_P, cmpByWeight>> a;
		return a;
	}
	return simplexList[dim];
}

std::vector<std::vector<std::set<simplexNode_P, cmpByWeight>>> simplexBase::getAllEdges(){
	return simplexList;
}
